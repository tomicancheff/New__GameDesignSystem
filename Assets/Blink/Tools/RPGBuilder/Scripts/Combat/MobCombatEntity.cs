...
namespace BLINK.RPGBuilder.Combat
{
    public class MobCombatEntity : CombatEntity
    {
        ...
        {
            foreach (var extraVitAction in npc.stats.Where(npcStat => npcStat.statID == stat.ID).SelectMany(npcStat => npcStat.vitalityActions))
            {
            ...
            {
                foreach (var extraVitAction in speciesREF.stats.Where(npcStat => npcStat.statID == stat.ID).SelectMany(npcStat => npcStat.vitalityActions))
                {
                    actions.Insert(0, extraVitAction);
                }
            }
