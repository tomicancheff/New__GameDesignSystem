            ...
            for (var i = 0; i < currentEntry.ranks.Count; i++)
            {
                ...
                if (currentEntry.ranks[i].ShowedInEditor)
                {
                        ...
                        RPGBuilderEditorFields.DrawTitleLabelExpanded("Hit Rules:", "", true);
                            ...
                            case RPGAbility.TARGET_TYPES.PROJECTILE:
                            {
                                ...
                                if (currentEntry.ranks[i].hitEffectUseSocket)
                                {
                                    ...
                                    int projSocketIndex = EditorGUILayout.Popup(
                                        RPGBuilderEditorUtility.GetTypeEntryIndex(allNodeSockets,
                                            currentEntry.ranks[i].hitEffectNodeSocket),
                                        RPGBuilderEditorUtility.GetTypeEntriesAsStringArray(allNodeSockets.ToArray()));
                                    currentEntry.ranks[i].hitEffectNodeSocket =
                                        (RPGBNodeSocket) allNodeSockets[projSocketIndex];
                                    EditorGUILayout.EndHorizontal();
                            ...
                            case RPGAbility.TARGET_TYPES.TARGET_PROJECTILE:
                                    ...
                                    .projectileTargetUseNodeSocket)
                                {
                                    int projSocketIndex = EditorGUILayout.Popup("Socket",
                                        RPGBuilderEditorUtility.GetTypeEntryIndex(allNodeSockets,
                                            currentEntry.ranks[i].projectileTargetNodeSocket),
                                        RPGBuilderEditorUtility.GetTypeEntriesAsStringArray(allNodeSockets.ToArray()));
                                    currentEntry.ranks[i].projectileTargetNodeSocket =
                                        (RPGBNodeSocket) allNodeSockets[projSocketIndex];
                                }
                    ...
                    if (RPGBuilderEditor.Instance.EditorFilters.abilityModuleSection.showCooldowns)
                    {
                        ...
                        {
                            RPGBuilderEditorFields.DrawHorizontalLabel("Cooldown Tag", "");
                            int cooldownTagIndex = EditorGUILayout.Popup(
                                RPGBuilderEditorUtility.GetTypeEntryIndex(allAbilityCooldownTag,
                                    currentEntry.ranks[i].abilityCooldownTag),
                                RPGBuilderEditorUtility.GetTypeEntriesAsStringArray(allAbilityCooldownTag.ToArray()));
                            currentEntry.ranks[i].abilityCooldownTag =
                                (RPGBAbilityCooldownTag) allAbilityCooldownTag[cooldownTagIndex];
                            EditorGUILayout.EndHorizontal();
                        }
