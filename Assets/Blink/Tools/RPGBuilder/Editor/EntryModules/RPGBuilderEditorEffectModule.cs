...
public class RPGBuilderEditorEffectModule : RPGBuilderEditorModule
{
    ...
    public override void DrawView()
    {
        ...
        if (RPGBuilderEditor.Instance.EditorFilters.effectModuleSection.showType)
        {
            ...
            RPGBuilderEditorFields.DrawHorizontalLabel("Effect Tag", "");
            int typeEntryIndex = EditorGUILayout.Popup(
                RPGBuilderEditorUtility.GetTypeEntryIndexWithNull(allEffectTags, currentEntry.EffectTag),
                RPGBuilderEditorUtility.GetTypeEntriesAsStringArray(allEffectTags.ToArray()));
            if (typeEntryIndex != -1)
            {
                currentEntry.EffectTag = (RPGBEffectTag) allEffectTags[typeEntryIndex];
            }
                    ...
                    switch (currentEntry.effectType)
                    {
                        ...
                        case RPGEffect.EFFECT_TYPE.DamageOverTime:
                        {
                            ...
                            if (!currentEntry.ranks[i].FlatCalculation)
                            {
                                ...
                                RPGBuilderEditorFields.DrawHorizontalLabel("Custom Damage Type", "");
                                int typeEntryIndex = EditorGUILayout.Popup(
                                    RPGBuilderEditorUtility.GetTypeEntryIndexWithNull(allDamageTypes, currentEntry.ranks[i].customDamageType),
                                    RPGBuilderEditorUtility.GetTypeEntriesAsStringArray(allDamageTypes.ToArray()));
                                if (typeEntryIndex != -1)
                                {
                                    currentEntry.ranks[i].customDamageType = (RPGBDamageType) allDamageTypes[typeEntryIndex];
                                }
                        ...
                        case RPGEffect.EFFECT_TYPE.InstantHeal:
                        {
                            ...
                            if (!currentEntry.ranks[i].FlatCalculation)
                            {
                                RPGBuilderEditorFields.DrawHorizontalLabel("Heal Type", "");
                                int typeEntryIndex = EditorGUILayout.Popup(
                                    RPGBuilderEditorUtility.GetTypeEntryIndexWithNull(allHealingTypes, currentEntry.ranks[i].customHealingType),
                                    RPGBuilderEditorUtility.GetTypeEntriesAsStringArray(allHealingTypes.ToArray()));
                                if (typeEntryIndex != -1)
                                {
                                    currentEntry.ranks[i].customHealingType = (RPGBHealingType) allHealingTypes[typeEntryIndex];
                                }
                                GUILayout.EndHorizontal();
                        ...
                        case RPGEffect.EFFECT_TYPE.Dispel:
                            ...
                            switch (currentEntry.ranks[i].dispelType)
                            {
                                ...
                                case RPGEffect.DISPEL_TYPE.EffectTag:
                                    RPGBuilderEditorFields.DrawHorizontalLabel("Effect Type", "");
                                    int typeEntryIndex = EditorGUILayout.Popup(
                                        RPGBuilderEditorUtility.GetTypeEntryIndex(allEffectTags,
                                            currentEntry.ranks[i].DispelEffectTag),
                                        RPGBuilderEditorUtility.GetTypeEntriesAsStringArray(allEffectTags.ToArray()));
                                    currentEntry.ranks[i].DispelEffectTag =
                                        (RPGBEffectTag) allEffectTags[typeEntryIndex];
                                    EditorGUILayout.EndHorizontal();
                                    break;
