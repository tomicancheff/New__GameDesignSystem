    ...
    public override void DrawView()
    {
        ...
        if (RPGBuilderEditor.Instance.EditorFilters.merchantTableModuleSection.showItems)
        {
            ...
            for (var a = 0; a < currentEntry.onSaleItems.Count; a++)
            {
                ...
                RPGCurrency currencyReference = (RPGCurrency)
                    RPGBuilderEditorUtility.GetEntryByID(currentEntry.onSaleItems[a].currencyID, "Currency");
                if (currencyReference != null)
                {
                    RPGCurrency currencyReference2 = (RPGCurrency)
                        RPGBuilderEditorUtility.GetEntryByID(currencyReference.convertToCurrencyID, "Currency");
                
                    if (currencyReference != null && currencyReference2 != null)
                    {
                        if (currencyReference.maxValue > 0 && currentEntry.onSaleItems[a].cost > currencyReference.maxValue)
                        {
                            currentEntry.onSaleItems[a].cost = currencyReference.maxValue;
                        }
                    }
                }
