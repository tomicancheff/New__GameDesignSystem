...
public class RPGBuilderEditorStatModule : RPGBuilderEditorModule
{
    ...
    public override void DrawView()
    {
        ...
        if (RPGBuilderEditor.Instance.EditorFilters.statModuleSection.showSetup)
        {
            ...
            int statCategory = EditorGUILayout.Popup(RPGBuilderEditorUtility.GetTypeEntryIndex(allStatCategories, currentEntry.StatCategory),
                RPGBuilderEditorUtility.GetTypeEntriesAsStringArray(allStatCategories.ToArray()));
            if (statCategory != -1)
            {
                currentEntry.StatCategory = (RPGBStatCategory) allStatCategories[statCategory];
            }
